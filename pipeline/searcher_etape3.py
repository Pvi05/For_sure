...
4 stratégies en cascade, par ordre de précision :
  ...
  B  — Followees : cherche dans les abonnements de l'auteur
       un post antérieur sur le même sujet.

  C  — Keywords : recherche globale par mots-clés (fallback large).

Priorité dans le tri final : reply > repost_parent > followee > keyword_search
"""
...
def _search_in_followees(
    author_did: str, keywords: dict, before_date: str,
    window_days: int, min_engagement: int,
    max_followees: int = 20, max_posts_per_followee: int = 10,
) -> list[BlueskyPost]:
    client = _get_client()
    results = []
    seen: set[str] = set()

    print(f"[Étape 3-B] 👥 Récupération des abonnements...")
    try:
        resp = client.app.bsky.graph.get_follows(
            params={"actor": author_did, "limit": max_followees}
        )
        followees = [f.did for f in (resp.follows or [])]
        print(f"[Étape 3-B]    → {len(followees)} abonnements")
    except Exception as e:
    ...
    for did in followees[:max_followees]:
        try:
            feed_resp = client.app.bsky.feed.get_author_feed(
                params={"actor": did, "limit": max_posts_per_followee}
            )
            for item in (feed_resp.feed or []):
                try:
                    post = _parse_post(item.post, strategy="followee")
                except Exception:
                    continue
                if post.uri in seen:
                    continue
    ...
    print(f"[Étape 3-B] ✅ {len(results)} post(s) chez les abonnements")
    return results
...
STRATEGY_ORDER = {"reply": 0, "repost_parent": 1,
                  "followee": 2, "keyword_search": 3, "unknown": 4}
...
def search_prior_posts(
    ...
    - B  (followees)      : cherche dans les abonnements de l'auteur un post antérieur
    ...
    # ── Stratégie B : abonnements ─────────────────────────────────────────
    print("[Étape 3] ── Stratégie B : abonnements")
    for p in _search_in_followees(author_did, keywords, before_date, window_days, min_engagement):
